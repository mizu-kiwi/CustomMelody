...
object MelodyMessage : Module(
    ...
    private val melody25 by StringSetting(
        "Melody 25% Message",
        "Melody 25%",
        64,
        desc = "Message sent at 25% melody progress."
    ).withDependency { melodyProgress }

    private val melody50 by StringSetting(
        "Melody 50% Message",
        "Melody 50%",
        64,
        desc = "Message sent at 50% melody progress."
    ).withDependency { melodyProgress }

    private val melody75 by StringSetting(
        "Melody 75% Message",
        "Melody 75%",
        64,
        desc = "Message sent at 75% melody progress."
    ).withDependency { melodyProgress }
    ...
    private val melodyGui by HUD(
        "Progress GUI",
        "Shows a gui with the progress of broadcasting odin users in melody.",
        true
    ) {
    ...
    private val showOwn by BooleanSetting(
        "Show Own",
        false,
    ...
    val melodyWebSocket = webSocket {
        onMessage {
            val (user, type, slot) = try {
                gson.fromJson(it, UpdateMessage::class.java)
            } catch (_: Exception) {
                return@onMessage
            }
    ...
    init {
        on<TerminalEvent.Open> {
            if (
                DungeonUtils.getF7Phase() != M7Phases.P3 ||
                terminal.type != TerminalTypes.MELODY ||
                mc.screen is TermSimGUI
            ) return@on
    ...
    private fun onSlotUpdate(packet: ClientboundContainerSetSlotPacket) {
        ...
        if (
            DungeonUtils.getF7Phase() != M7Phases.P3 ||
            term.type != TerminalTypes.MELODY ||
            mc.screen is TermSimGUI
        ) return
        ...
        if (item == Items.LIME_TERRACOTTA) {
            ...
            if (melodyProgress) {
                getClayMessage(position)
                    ?.takeIf { it.isNotBlank() }
                    ?.let { sendCommand("pc $it") }
            }
    ...
    private fun getClayMessage(slot: Int): String? = when (slot) {
        2 -> melody25
        3 -> melody50
        4 -> melody75
        else -> null
    ...
    private fun update(type: Int, slot: Int): String =
        gson.toJson(UpdateMessage(mc.user.name, type, slot))
